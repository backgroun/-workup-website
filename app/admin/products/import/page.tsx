...
const COLUMNS = [
  ...
  { key: "tagline",       label: "한줄소개",        required: false },
  { key: "features",      label: "주요특징(;구분)", required: false },
  { key: "featureTags",   label: "기능태그(;구분)", required: false },
  { key: "jobSites",      label: "현장추천(;구분)", required: false },
  { key: "seasons",       label: "시즌(;구분)",     required: false },
  { key: "mainExpose",    label: "메인노출(;구분)", required: false },
...
function parseRow(row: Record<string, unknown>, idx: number): Partial<Product> & { id: string; _row: number; _error?: string } {
  ...
  return {
    ...
    tagline: String(row["한줄소개"] ?? row["tagline"] ?? "").trim(),
    features: toArr(row["주요특징(;구분)"] ?? row["features"]),
    featureTags: toArr(row["기능태그(;구분)"] ?? row["featureTags"]),
    jobSites: toArr(row["현장추천(;구분)"] ?? row["jobSites"]),
    seasons: toArr(row["시즌(;구분)"] ?? row["seasons"]) as Product["seasons"],
    mainExpose: toArr(row["메인노출(;구분)"] ?? row["mainExpose"]) as Product["mainExpose"],
    isNew: toArr(row["메인노출(;구분)"] ?? row["mainExpose"]).includes("신상품"),
