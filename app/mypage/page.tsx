...
function WithdrawModal({ onClose, onWithdrawn }: { onClose: () => void; onWithdrawn: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(d.error ?? "회원 탈퇴에 실패했습니다.");
      }
    } catch {
      setError("네트워크 오류가 발생했습니다. 잠시 후 다시 시도해주세요.");
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm" onClick={e => e.stopPropagation()}>
        {step === "confirm" ? (
          <>
            ...
            <div className="px-6 py-6 space-y-4">
              <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 space-y-2">
                <p className="text-sm font-semibold text-amber-800">탈퇴 전 꼭 확인하세요</p>
                <ul className="text-xs text-amber-700 space-y-1 list-disc list-inside">
                  <li>문의 내역 등 기존 데이터는 복구되지 않습니다.</li>
              ...
              <div className="flex gap-2.5 pt-1">
                ...
                <button onClick={() => setStep("form")}
                  className="flex-1 py-3 bg-red-500 text-white text-sm font-semibold rounded-lg hover:bg-red-600 transition-colors">
                  탈퇴 진행
                </button>
              </div>
            </div>
          ...
          <>
            <div className="px-6 py-5 border-b border-gray-100">
              <h2 className="text-base font-bold text-gray-900">회원 탈퇴 확인</h2>
            </div>
            <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
              {error && (
                <div className="px-3.5 py-2.5 bg-red-50 border border-red-200 text-red-700 text-xs rounded-lg">{error}</div>
              ...
              <div>
                ...
                <PasswordInput
                  ...
                  className="w-full border border-gray-200 px-3 py-2.5 text-sm rounded-lg focus:outline-none focus:border-red-400 focus:ring-2 focus:ring-red-400/10 transition-colors"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1.5">탈퇴 사유 (선택)</label>
                <select
                  value={reason} onChange={e => setReason(e.target.value)}
                  className="w-full border border-gray-200 px-3 py-2.5 text-sm rounded-lg focus:outline-none focus:border-[#303236] transition-colors bg-white"
                >
              ...
              <div className="flex gap-2.5 pt-1">
                ...
                <button type="submit" disabled={saving}
                  className="flex-1 py-3 bg-red-500 text-white text-sm font-semibold rounded-lg hover:bg-red-600 disabled:opacity-50 transition-colors">
                  {saving ? "처리 중..." : "탈퇴 완료"}
                </button>
              </div>
            </form>
