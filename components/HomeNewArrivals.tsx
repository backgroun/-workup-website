...
export default function HomeNewArrivals() {
  ...
  return (
    <section className="bg-white pt-10 pb-8 md:pt-12 md:pb-14 overflow-x-hidden">
      <div className="px-[15px] md:px-[70px]">
        <h2 className="text-[19px] md:text-2xl font-[600] md:font-[500] text-[#303236] mb-5 tracking-tight">{cfg.title}</h2>
          ...
          >
            {newItems.map((p) => (
              <Link key={p.id} href={`/products/${p.id}`} className="flex-shrink-0 group" style={{ width: "280px" }}>
                ...
                >
                  {p.imageUrl ? (
                    <img src={ikResize(p.imageUrl, 560)} alt={p.name} loading="lazy" decoding="async"
                ...
                <div className="pt-3">
                  ...
                  <p className="text-[13px] text-[#303236] leading-snug line-clamp-2 mb-1 group-hover:underline underline-offset-4 decoration-1">
          ...
          >
            {newItems.map((p) => (
              <Link key={p.id} href={`/products/${p.id}`} className="flex-shrink-0 group" style={{ width: "220px" }}>
                ...
                >
                  {p.imageUrl ? (
                    <img src={ikResize(p.imageUrl, 440)} alt={p.name} loading="lazy" decoding="async" className="w-full h-full object-cover" />
                  ) : (
