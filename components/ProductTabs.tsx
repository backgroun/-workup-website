  ...
  return (
    <>
      {guide.guideImage && (
        <div className="mb-6 flex justify-center">
          <div className="relative inline-block">
            ...
            <img src={ikSrc(guide.guideImage, 800)} alt={`${productName} 사이즈 측정 위치 안내`} className="block h-auto max-w-full grayscale opacity-40" loading="lazy" />
      ...
      {hasImage ? (
        <div>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={ikSrc(guide.image!, 800)} alt={`${productName} 사이즈 가이드`} className="block h-auto max-w-full" loading="lazy" />
          ...
          {(product.detailBlocks ?? []).some((b) => b.imageUrl) && (
            <div className="space-y-3 max-w-3xl mx-auto mb-10">
              {product.detailBlocks!.filter((b) => b.imageUrl).map((b, i) => (
                // eslint-disable-next-line @next/next/no-img-element
                <img key={b.id ?? i} src={ikSrc(b.imageUrl!, 1200)} alt={`${product.name} 상세 ${i + 1}`} className="block w-full h-auto bg-[#f4f4f4]" loading="lazy" />
              ))}
            </div>
          )}
