...
const BANNER_SLIDES = [
  { id: 1, bg: "#B5152B", accent: "#ff550c", tag: "2026 Summer", title: "땀 흘려도 쾌적한\n여름 작업복", desc: "흡한속건 · 항균 · UPF50+ 자외선 차단", cta: "여름 컬렉션 보기", href: "/products" },
  { id: 2, bg: "#1A2B4A", accent: "#ff550c", tag: "SAFETY GEAR", title: "현장을 지키는\n안전용품", desc: "안전화 · 안전모 · 보호구 · 안전조끼", cta: "안전용품 보러가기", href: "/products" },
  { id: 3, bg: "#2d2d2d", accent: "#ff550c", tag: "SET-UP", title: "현장부터 퇴근 후까지\n셋업 컬렉션", desc: "상하의 세트로 스타일과 기능을 동시에", cta: "셋업 보러가기", href: "/products" },
];


function ProductsBanner({ height = 200 }: { height?: number }) {
  const [current, setCurrent] = useState(0);
  const [paused, setPaused] = useState(false);
  const touchStartX = useRef<number | null>(null);

  const next = useCallback(() => setCurrent((c) => (c + 1) % BANNER_SLIDES.length), []);
  const prev = useCallback(() => setCurrent((c) => (c - 1 + BANNER_SLIDES.length) % BANNER_SLIDES.length), []);
  ...
  const isTall = height >= 380;

  return (
    <div
      className="relative overflow-hidden w-full select-none"
      style={{ height: `${height}px`, backgroundColor: slide.bg, transition: "background-color 0.5s ease" }}
    ...
    >
      {BANNER_SLIDES.map((s, i) => (
        <div key={s.id}
          className={`absolute inset-0 flex items-center ${isTall ? "px-[70px]" : "px-[15px] md:px-8"}`}
          style={{ opacity: i === current ? 1 : 0, transition: "opacity 0.6s ease", pointerEvents: i === current ? "auto" : "none" }}>
          <div className="absolute right-0 top-0 h-full flex items-center pr-8 opacity-[0.05] select-none pointer-events-none overflow-hidden">
            <span className="text-white font-black leading-none" style={{ fontSize: isTall ? "320px" : "160px" }}>WU</span>
          </div>
          <div className="relative z-10">
            <p className={`font-semibold tracking-[0.22em] uppercase mb-3 ${isTall ? "text-[12px]" : "text-[10px]"}`} style={{ color: s.accent }}>{s.tag}</p>
            <h2 className={`font-bold text-white leading-tight mb-3 whitespace-pre-line ${isTall ? "text-[42px]" : "text-[24px]"}`}>{s.title}</h2>
            <p className={`text-white/60 mb-6 ${isTall ? "text-[15px]" : "text-[13px]"}`}>{s.desc}</p>
            <Link href={s.href}
              className={`inline-flex items-center gap-1.5 font-semibold tracking-widest hover:opacity-80 transition-opacity ${isTall ? "text-[13px] px-7 py-3.5" : "text-[12px] px-5 py-2.5"}`}
      ...
      >
        ...
        >
          <svg
            ...
            fill={isWishlisted(product.id) ? "#B5152B" : "none"}
            stroke={isWishlisted(product.id) ? "#B5152B" : "white"}
            strokeWidth={1.5}
            viewBox="0 0 24 24"
          >
