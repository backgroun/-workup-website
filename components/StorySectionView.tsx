...
function StoryImagePanel({ src, alt }: { src?: string; alt: string }) {
  if (src) {
    return (
      ...
      <img src={ikResize(src, 1200)} alt={alt} loading="lazy" decoding="async" className="w-full h-full object-cover object-top" style={{ aspectRatio: IMG_RATIO }} />
    );
  }
  return (
...
// bare 이면 원형 배경 없이 아이콘만(= WORK LIFE WEAR와 동일한 톤) 렌더링한다.
  ...
  return (
    <div className="relative inline-block">
      <button type="button" onClick={(e) => { e.stopPropagation(); setOpen((v) => !v); }}
        className={`group/icon relative ring-offset-2 hover:ring-2 hover:ring-blue-400 transition-shadow ${bare ? "rounded-lg" : "rounded-full"}`}>
        {circle}
        <span className={`pointer-events-none absolute inset-0 bg-black/0 group-hover/icon:bg-black/30 flex items-center justify-center transition-colors ${bare ? "rounded-lg" : "rounded-full"}`}>
            ...
            <div className="w-full lg:ml-[2.03%] lg:flex-1 lg:self-stretch lg:relative">
              <div className="lg:absolute lg:inset-x-0 lg:top-[80px] lg:bottom-[80px]">
                <ImgPick onPick={pickInto("image_url")} has={!!section.image_url} className="lg:h-full">
                  {section.image_url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={ikResize(section.image_url, 1000)} alt={section.heading || "브랜드 선언"} loading="lazy" decoding="async"
            ...
            <div className={`${GRID} md:grid-cols-2`}>
              ...
              <div className={imageLeft ? "md:order-1" : ""}>
                <ImgPick onPick={pickInto("image_url")} has={!!section.image_url}>
                  {section.image_url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={ikResize(section.image_url, 900)} alt={section.heading || "창업 스토리"} loading="lazy" decoding="async" className="w-full h-full object-cover object-top" style={{ aspectRatio: IMG_RATIO }} />
                  ) : (
            ...
            {images.map((img, i) => (
              <div key={i} className="group/item relative">
                ...
                <ImgPick onPick={edit ? () => edit.pickImage((url) => setImg(i, url)) : undefined} has={!!img.url}>
                  {img.url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={ikResize(img.url, 640)} alt={img.alt || ""} loading="lazy" decoding="async" className="w-full aspect-square object-cover" />
                  ) : (
            ...
            <div className={`${GRID} ${imageLeft ? COLS_IMG_TEXT : COLS_TEXT_IMG}`}>
              <div className={imageLeft ? "md:order-1" : "md:order-2"}>
                <ImgPick onPick={pickInto("image_url")} has={!!section.image_url}>
                  {section.image_url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={ikResize(section.image_url, 900)} alt={section.heading || "문제 제기"} loading="lazy" decoding="async" className="w-full h-full object-cover object-top grayscale"
              ...
              images.map((img, i) => (
                <div key={i} className="group/item relative h-full">
                  ...
                  <ImgPick onPick={edit ? () => edit.pickImage((url) => setImg(i, url)) : undefined} has={!!img.url}>
                    {img.url ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={ikResize(img.url, 640)} alt={img.alt || ""} loading="lazy" decoding="async" className="w-full h-full object-cover" />
                    ) : (
