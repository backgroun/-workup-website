// ─────────────────────────────────────────────────────────────────────────────
// Cloudinary → ImageKit 자산 이전 도구
//
// Cloudinary workup/ 폴더의 모든 자산(이미지·영상·PDF)을 ImageKit으로 재업로드하고,
// (--apply-db 사용 시) Supabase 전 테이블의 옛 Cloudinary URL을 새 ImageKit URL로 교체한다.
//
// 단계:
//   1) Cloudinary workup/ 폴더의 모든 자산 나열 (image·video·raw)
//   2) 각 자산을 다운로드해 ImageKit에 같은 폴더 구조로 재업로드
//      → 결과는 scripts/.cloudinary-imagekit-map.json 에 저장(중단 후 재개 가능 — 이미 옮긴 자산은 건너뜀)
//   3) --apply-db 를 줄 때만: brand_catalogs 는 pdf_public_id/pdf_file_id/pdf_url/thumbnail_url을
//      새 ImageKit 정보로 갱신하고, 나머지 테이블은 옛 URL 문자열을 새 URL로 일괄 치환
//
// 실행:
//   node scripts/cloudinary-imagekit-migrate.mjs              # 1) 업로드만 (DB는 그대로, 안전)
//   node scripts/cloudinary-imagekit-migrate.mjs --apply-db   # 2) 위 결과로 Supabase URL까지 갱신
//   node scripts/cloudinary-imagekit-migrate.mjs --prefix workup/brands   # 특정 폴더만 대상
//
// 안전장치:
//   - 업로드(1단계)는 기존 Cloudinary 자산을 건드리지 않는 추가(added-only) 작업이라 안전하다.
//   - DB 갱신(--apply-db)은 매핑 파일이 있어야만 실행되며, 실행 전 몇 개가 바뀌는지 미리 보여준다.
//   - 핵심 테이블(products·brand_catalogs·site_settings) 조회 실패 시 DB 갱신을 거부한다.
// ─────────────────────────────────────────────────────────────────────────────
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { createClient } from "@supabase/supabase-js";
import cloudinaryPkg from "cloudinary";
import ImageKit, { toFile } from "@imagekit/nodejs";

const cloudinary = cloudinaryPkg.v2;
const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const MAP_PATH = path.join(ROOT, "scripts", ".cloudinary-imagekit-map.json");

// ── CLI 인자 ──
const argv = process.argv.slice(2);
const APPLY_DB = argv.includes("--apply-db");
const prefixArg = argv.find((a) => a.startsWith("--prefix="))?.split("=")[1];
const prefixIdx = argv.indexOf("--prefix");
const PREFIX = (prefixArg || (prefixIdx >= 0 ? argv[prefixIdx + 1] : "") || "workup/").replace(/\/?$/, "/");

// ── .env.local 로드 ──
const env = {};
try {
  for (const line of fs.readFileSync(path.join(ROOT, ".env.local"), "utf8").split(/\r?\n/)) {
    const m = line.match(/^([A-Z0-9_]+)=(.*)$/);
    if (m) env[m[1]] = m[2];
  }
} catch {
  console.log("❌ .env.local 을 찾을 수 없습니다. (Cloudinary·ImageKit·Supabase 키 필요)");
  process.exit(1);
}
for (const k of ["CLOUDINARY_CLOUD_NAME", "CLOUDINARY_API_KEY", "CLOUDINARY_API_SECRET", "IMAGEKIT_PRIVATE_KEY", "IMAGEKIT_URL_ENDPOINT", "NEXT_PUBLIC_SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY"]) {
  if (!env[k]) { console.log(`❌ .env.local 에 ${k} 가 없습니다.`); process.exit(1); }
}
cloudinary.config({
  cloud_name: env.CLOUDINARY_CLOUD_NAME,
  api_key: env.CLOUDINARY_API_KEY,
  api_secret: env.CLOUDINARY_API_SECRET,
});
const imagekit = new ImageKit({ privateKey: env.IMAGEKIT_PRIVATE_KEY });
const URL_ENDPOINT = env.IMAGEKIT_URL_ENDPOINT.replace(/\/$/, "");
const sb = createClient(env.NEXT_PUBLIC_SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY);

// data/brandCatalogs.ts 의 brandCoverUrl 과 동일한 문법(빌드 없이 재사용하기 위해 인라인).
function brandCoverUrl(filePath) {
  return `${URL_ENDPOINT}/tr:pg-1,w-300,h-420,c-maintain_ratio,f-auto,q-auto/${filePath}/ik-thumbnail.jpg`;
}

const fmtMB = (bytes) => (bytes / 1024 / 1024).toFixed(2) + " MB";

// ── 1) Cloudinary 자산 나열 ──
async function listAll(resourceType) {
  const out = [];
  let next;
  do {
    const res = await cloudinary.api.resources({
      resource_type: resourceType,
      type: "upload",
      prefix: PREFIX,
      max_results: 500,
      next_cursor: next,
    });
    for (const r of res.resources) {
      if (r.public_id.startsWith(PREFIX)) {
        out.push({ public_id: r.public_id, resource_type: resourceType, format: r.format, bytes: r.bytes || 0, secure_url: r.secure_url });
      }
    }
    next = res.next_cursor;
  } while (next);
  return out;
}

// ── 2) 자산 하나를 ImageKit으로 재업로드 ──
async function migrateOne(asset) {
  const res = await fetch(asset.secure_url);
  if (!res.ok) throw new Error(`다운로드 실패 (${res.status})`);
  const buf = Buffer.from(await res.arrayBuffer());
  const dir = path.posix.dirname(asset.public_id);
  const base = path.posix.basename(asset.public_id);
  const fileName = asset.format ? `${base}.${asset.format}` : base;
  const folder = "/" + dir;
  const result = await imagekit.files.upload({
    file: await toFile(buf, fileName),
    fileName,
    folder,
    useUniqueFileName: false,
  });
  return { newUrl: result.url, newFilePath: result.filePath, newFileId: result.fileId };
}

// ── 3) Supabase 전 테이블에서 옛 URL → 새 URL 일괄 치환 (brand_catalogs 제외) ──
const CORE_TABLES = ["products", "site_settings"];
const OTHER_TABLES = [
  "catalog_pages", "stores", "store_events", "store_jobs",
  "hero_slides", "brands", "manufacturers", "members", "pixel_settings",
  "inquiries", "inquiry_dummies",
];

async function replaceAcrossTables(urlMap) {
  let coreFailed = false;
  let totalUpdated = 0;
  for (const table of [...CORE_TABLES, ...OTHER_TABLES]) {
    const { data, error } = await sb.from(table).select("*");
    if (error) {
      const missing = /does not exist|schema cache|42P01/i.test(error.message);
      console.log(`  ⚠ ${table}: ${error.message}${missing ? " (없는 테이블 → 건너뜀)" : ""}`);
      if (!missing && CORE_TABLES.includes(table)) coreFailed = true;
      continue;
    }
    let changed = 0;
    for (const row of data ?? []) {
      let json = JSON.stringify(row);
      let touched = false;
      for (const [oldUrl, newUrl] of urlMap) {
        if (json.includes(oldUrl)) { json = json.split(oldUrl).join(newUrl); touched = true; }
      }
      if (!touched) continue;
      const next = JSON.parse(json);
      delete next.created_at; delete next.updated_at; // 트리거/기본값에 맡김
      const { error: upErr } = await sb.from(table).update(next).eq("id", row.id);
      if (upErr) console.log(`  ✗ ${table}#${row.id}: ${upErr.message}`);
      else changed++;
    }
    if (changed) { console.log(`  · ${table}: ${changed}행 갱신`); totalUpdated += changed; }
  }
  return { totalUpdated, coreFailed };
}

// ── 4) brand_catalogs 전용 처리 ──
async function migrateBrandCatalogs(assetByPublicId) {
  const { data, error } = await sb.from("brand_catalogs").select("*");
  if (error) { console.log(`  ⚠ brand_catalogs: ${error.message}`); return 0; }
  let changed = 0;
  for (const row of data ?? []) {
    const info = assetByPublicId.get(row.pdf_public_id);
    if (!info) continue; // 이미 ImageKit 자산이거나 매핑 없음
    const { error: upErr } = await sb.from("brand_catalogs").update({
      pdf_public_id: info.newFilePath,
      pdf_file_id: info.newFileId,
      pdf_url: info.newUrl,
      thumbnail_url: brandCoverUrl(info.newFilePath),
    }).eq("id", row.id);
    if (upErr) console.log(`  ✗ brand_catalogs#${row.id}: ${upErr.message}`);
    else changed++;
  }
  if (changed) console.log(`  · brand_catalogs: ${changed}행 갱신`);
  return changed;
}

(async () => {
  console.log(`\n🔎 대상 폴더: ${PREFIX}  |  모드: ${APPLY_DB ? "🚀 업로드 + DB 갱신(--apply-db)" : "📤 업로드만 (DB 미변경)"}\n`);

  // 기존 매핑 로드 (재개용)
  let existingMap = [];
  try { existingMap = JSON.parse(fs.readFileSync(MAP_PATH, "utf8")); } catch { /* 최초 실행 */ }
  const done = new Map(existingMap.map((m) => [m.public_id, m]));

  console.log("① Cloudinary 자산 나열…");
  const assets = [
    ...(await listAll("image")),
    ...(await listAll("video")),
    ...(await listAll("raw")),
  ];
  const totalBytes = assets.reduce((s, a) => s + a.bytes, 0);
  console.log(`   → 총 ${assets.length}개 자산 (${fmtMB(totalBytes)})  |  이미 이전됨: ${done.size}개\n`);

  console.log("② ImageKit으로 재업로드…");
  const toProcess = assets.filter((a) => !done.has(a.public_id));
  let ok = 0, fail = 0;
  for (const asset of toProcess) {
    try {
      const { newUrl, newFilePath, newFileId } = await migrateOne(asset);
      done.set(asset.public_id, { public_id: asset.public_id, oldUrl: asset.secure_url, newUrl, newFilePath, newFileId });
      ok++;
      if (ok % 25 === 0) {
        fs.writeFileSync(MAP_PATH, JSON.stringify([...done.values()], null, 2));
        console.log(`   … ${ok}/${toProcess.length} 진행 중 (중간 저장됨)`);
      }
    } catch (e) {
      fail++;
      console.log(`   ✗ ${asset.public_id}: ${e.message || e}`);
    }
  }
  fs.writeFileSync(MAP_PATH, JSON.stringify([...done.values()], null, 2));
  console.log(`   → 신규 업로드 ${ok}개 성공, ${fail}개 실패 (전체 이전 완료: ${done.size}/${assets.length})`);
  console.log(`   📄 매핑 저장: ${path.relative(ROOT, MAP_PATH)}\n`);

  if (!APPLY_DB) {
    console.log("👀 DB는 아직 그대로입니다. 위 매핑을 확인한 뒤, 실제 사이트에 반영하려면:");
    console.log("   node scripts/cloudinary-imagekit-migrate.mjs --apply-db\n");
    return;
  }

  console.log("③ brand_catalogs 갱신…");
  const assetByPublicId = new Map([...done.values()].map((m) => [m.public_id, m]));
  const brandChanged = await migrateBrandCatalogs(assetByPublicId);

  console.log("\n④ 나머지 테이블 URL 일괄 치환…");
  const urlMap = [...done.values()].map((m) => [m.oldUrl, m.newUrl]);
  const { totalUpdated, coreFailed } = await replaceAcrossTables(urlMap);

  if (coreFailed) {
    console.log("\n❌ 핵심 테이블 조회에 실패해 일부만 반영됐을 수 있습니다. 결과를 꼭 확인하세요.");
  }
  console.log(`\n✅ 완료 — brand_catalogs ${brandChanged}행 + 기타 테이블 ${totalUpdated}행 갱신됨.`);
  console.log("   사이트를 새로고침해 이미지·영상·PDF가 정상 표시되는지 확인해주세요.");
})().catch((e) => { console.error("\n❌ 오류:", e.message || e); process.exit(1); });
