...
-- 관리자(/admin/catalog/brands)에서 PDF 업로드 → Cloudinary가 페이지 이미지로 변환 → /brands 뷰어에서 플립북으로 노출.
-- Supabase SQL Editor에서 1회 실행.
CREATE TABLE IF NOT EXISTS brand_catalogs (
  id            TEXT PRIMARY KEY,
  brand_name    TEXT NOT NULL DEFAULT '',
  pdf_public_id TEXT,             -- Cloudinary public_id (페이지 이미지 생성용)
  pdf_url       TEXT,             -- 원본 PDF secure_url (대체 보기용)
